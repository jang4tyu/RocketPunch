...
public class ResolutionUtil : MonoBehaviour
{
    ...
    public static Camera CreateBlackCamera(int depth = -10, bool dontDestroy = true)
    {
        ...
        backCamObj.GetComponent<Camera>().clearFlags = CameraClearFlags.SolidColor;
        backCamObj.GetComponent<Camera>().backgroundColor = Color.black;
        backCamObj.GetComponent<Camera>().cullingMask = 0;
        backCamObj.GetComponent<Camera>().depth = depth;

        if (dontDestroy)
            DontDestroyOnLoad(backCamObj);

        return backCamObj.GetComponent<Camera>();
    }
}
